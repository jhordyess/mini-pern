...
export default class ProductCtl extends BaseController {
  ...
  getProductInfo = async (req) => {
    ...
    if (typeof id === "undefined")
      throw response.throw({ message: "Invalid identifier" });
    let product = {};
    switch (type) {
      case "basic":
  ...
  createProduct = async (req, res) => {
    ...
    if (typeof product.id === "undefined")
      throw response.throw({ message: "Can't create sku!" });
    await this.disconnect();
    return response.normal({
      statusCode: 201,
  ...
  deleteProduct = async (req) => {
    ...
    if (typeof ids === "undefined")
      throw response.throw({ message: "Invalid selection" });
    ...
    if (count !== ids.length)
      throw response.throw({ message: "Delete uncomplete" });
    return response.normal({
